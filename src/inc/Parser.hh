...
namespace ExpUtils {
  ...
  template <typename T> bool convert(const std::string &s, T &dat) {
  ...
  template <typename T>
  bool convert(const std::string &s, std::vector<T> &datVec,
               std::string delim = " ") {
    std::vector<std::string> strVec = _misc::stringExplode(s, delim);
    datVec.resize(strVec.size());
    bool ret = true;
    for (size_t i = 0; i < datVec.size(); i++) {
  ...
  bool convert(const std::string &s, std::array<T, N> &datVec,
               std::string delim = " ") {
    std::vector<std::string> strVec = _misc::stringExplode(s, delim);
    // datVec.resize(strVec.size());
    if (strVec.size() != N)
  ...
  namespace _misc {
    ...
    template <typename T>
    bool ParseList(const std::vector<std::string> &_slist, size_t i, T &dat) {
      return convert(_slist[i], dat);
    }

    template <typename First, typename... Params>
    bool ParseList(const std::vector<std::string> &_slist, size_t i, First &firstparam,
                   Params &... params) {
  ...
  bool ParseList(const std::vector<std::string> &_slist,
                 std::tuple<Types...> &t) {
    return _misc::ParseList(
                            _slist, t,
                            _misc::make_index_sequence<std::tuple_size<std::tuple<Types...>>{}>{});
  }

  template <typename... Types>
  std::tuple<Types...> ParseList(const std::vector<std::string> &_slist) {
    std::tuple<Types...> ret;
    if (!_misc::ParseList(_slist, ret,
                          _misc::make_index_sequence<
  ...
  template <typename... Types>
  bool ParseString(const std::string &str, std::tuple<Types...> &t,
                   std::string delim = " ") {
  ...
  template <typename... Types>
  std::tuple<Types...> ParseString(const std::string &str,
                                   std::string delim = " ") {
    std::tuple<Types...> ret;
