...
TEST (ArgParserTest, orderedTest_otherEarly) {
  ...
  try{
    auto args = argParse.parse(std::vector<std::string>{"versionTest","1","--test3", "1"});
  } catch (std::runtime_error& e){
    // std::cerr<<e.what()<<std::endl;
    EXPECT_EQ(std::string(e.what()),"Format String Coming soon....");
    exception = true;
...
TEST(ArgParserTest, storeTest_Multi){
  ...
  auto args = argParse.parse(std::vector<std::string>{"storeTest", "--test", "1", "2"});
  std::vector<int> v{{1,2}};
  std::array<int,2> a{{1,2}};

  //We exect to recieve the first when converting to a single integer, and then both when converting to vector.
  EXPECT_EQ(args.retrieve<int>("test"),1);
...
TEST(ArgParserTest, storeTest_Exceptions){
  ...
  try{
    ...
    exception = true;
    EXPECT_EQ(std::string(e.what()),"Something went wrong with parsing (probably not the right type) test");
  }
  EXPECT_TRUE(exception);
...
TEST(ArgParserTest, booleanFunctions){
  ...
  ExpUtils::ArgParser argParse{"v0.1"};
  argParse.addArgument("runThis","rt", ExpUtils::ArgParser::ArgType::store_true);
  argParse.addArgument("dontRunThis", "drt", ExpUtils::ArgParser::ArgType::store_false);
...
TEST(ArgParserTest, appendTest){
  ...
  auto args = argParse.parse({"appendTest", "-a", "1", "-s","2","3","-a","4","5","6"});
...
TEST(ArgParserTest, multiArgument){
  ...
  argParse.addArgument("storeTrue", "st", ExpUtils::ArgParser::ArgType::store_true);
  argParse.addArgument("store", "s", false);

  auto args = argParse.parse({"multiArgument", "-s", "1", "3", "5", "-st", "--test", "10.0"});
